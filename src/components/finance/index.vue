...
<el-table-column show-overflow-tooltip align="center" prop="accountName" label="对方名称" width="140"></el-table-column>
<el-table-column show-overflow-tooltip v-if="!isRentSystem" align="center" prop="houseArray" min-width="140" label="楼盘名称">
    <template slot-scope="scope">{{getCommunityName (scope.row.houseArray)}}</template>
</el-table-column>
<el-table-column v-if="isRentSystem" show-overflow-tooltip align="center" prop="roomAddress" label="房间地址" min-width="140"></el-table-column>
<el-table-column v-if="!isRentSystem" show-overflow-tooltip align="center" prop="houseArray" label="房间号" min-width="140">
...
<el-table-column show-overflow-tooltip align="center" prop="billType" :formatter="billState" label="账单类型" width="90"></el-table-column>
<el-table-column show-overflow-tooltip align="center" prop="amount" label="账单金额" width="100"></el-table-column>
<el-table-column show-overflow-tooltip align="center" prop="factAmount" label="实收/付费用" min-width="100"></el-table-column>
<el-table-column show-overflow-tooltip align="center" prop="overDays" min-width="140" :formatter="overDaysState" label="逾期状态"></el-table-column>
<el-table-column show-overflow-tooltip align="center" label="计费周期" width="200">
    <template slot-scope="scope">{{scope.row.feeStartDate | timeToStr('YYYY-MM-DD')}}~{{scope.row.feeEndDate | timeToStr('YYYY-MM-DD')}}</template>
</el-table-column>
<el-table-column show-overflow-tooltip align="center" label="应收/应付时间" width="140">
    <template slot-scope="scope">{{scope.row.payEndDate || scope.row.payStartDate | timeToStr('YYYY-MM-DD HH:mm')}}</template>
</el-table-column>
<el-table-column show-overflow-tooltip align="center" prop="settleStatus" :formatter="formatSettleStatus" label="结清状态" width="90"></el-table-column>
<el-table-column show-overflow-tooltip align="center" prop="billSource" label="账单来源" width="120">
    <template slot-scope="scope">{{ billSourceType[scope.row.billSource] || '未知' }}</template>
</el-table-column>
...
<side-popup :visible.sync="billInfoVisible" :modal="false" title="账单信息" width="800px">
    ...
    <bill-info v-if="billInfoVisible" :billCodeInfo="billCodeInfo" :billIdInfo="billIdInfo" @submit="tableReload"></bill-info>
</side-popup>
<!-- 操作弹出框 -->
<el-dialog :title="dialogInfo.title" :visible.sync="dialogInfo.visible" v-dialogDrag :append-to-body="true" :close-on-click-modal="false" top="20px" :width="dialogInfo.width">
    <!--dialogInfo.component 切换组件
                dialogInfo prop数据
            @submit  刷新列表-->
    <component :queryForm="queryForm" :is="dialogInfo.component" :dialogInfo="dialogInfo" v-if="dialogInfo.visible" @submit="tableReload"></component>
</el-dialog>
</div>
</template>
<script>
        ...
        methods: {
            ...
            delItem(row) {
                ...
                this.$confirm("此操作将永久删除, 是否继续?", "提示", {
                    ...
                    .then(() => {
                        this.$api.seeFinanceService.delBillList({
                                ids: ids
                            })
                            .then(res => {
                                // 刷新列表
